...
class Controller_elfinder extends Controller
{
	public function action_connector()
	{
		...
		$opts = array(
			'root'            => DOCROOT . 'uploads/' . Session::get('lang_prefix') . '/content/',                       // path to root directory
			'URL'             => Uri::create('uploads/' . Session::get('lang_prefix') . '/content/'), // root directory URL
		...
		$elfinder = new elfinder\elFinder($opts);
		$elfinder->run();
	}
}